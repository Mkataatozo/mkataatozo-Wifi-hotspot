...
export const MobilePaymentModal: React.FC<MobilePaymentModalProps> = ({
  ...
  const [paymentStep, setPaymentStep] = useState<'input' | 'waiting_approval' | 'verifying' | 'success' | 'failed' | 'timeout'>('input');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      ...
      >
        ...
        {/* STEP 5: Timeout (customer never responded to the USSD prompt in time) */}
        {paymentStep === 'timeout' && (
          <div className="mt-4 py-4 text-center space-y-4">
            ...
            <div>
              <h4 className="font-bold text-slate-900 text-base">
                {lang === 'sw' ? 'Muda wa Ombi Umeisha' : 'Request Timed Out'}
              </h4>
              <p className="text-xs text-slate-600 mt-1">
                {lang === 'sw'
                  ? 'Hukujibu ombi la malipo kwa wakati. Tafadhali jaribu tena.'
                  : "You didn't respond to the payment prompt in time. Please try again."}
              </p>
            </div>
            <button
              onClick={() => setPaymentStep('input')}
